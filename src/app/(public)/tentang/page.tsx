      ...
      <section className="section">
        <div className={`container ${styles.grid}`}>
          <div className={styles.content}>
            ...
            <p>
              {storeInfo.about_history || (lang === 'id' 
                ? `Didirikan pada tahun ${storeInfo.since}, ${storeInfo.name} telah menjadi destinasi terpercaya bagi para pecinta dan investor emas.`
                : `Established in ${storeInfo.since}, ${storeInfo.name} has become a trusted destination for gold enthusiasts and investors.`)}
            </p>
            <p>{storeInfo.about_extra || dict.abt_history_desc}</p>
            
            <h2 style={{ marginTop: "40px" }}>{dict.abt_vision_title}</h2>
            <p>{storeInfo.about_vision || dict.abt_vision_desc}</p>
          ...
          <div className={styles.sidebar}>
            <div className={`glass-card ${styles.certCard}`}>
              ...
              <ul className={styles.certList}>
                {((storeInfo.about_strengths && storeInfo.about_strengths.length) ? storeInfo.about_strengths : (storeInfo.certifications || [])).map((cert: string, index: number) => (
                  <li key={index}>
      ...
      <section className="section" style={{ paddingBottom: 0 }}>
        <div className="container" style={{ marginBottom: "40px" }}>
          ...
          <div style={{ width: "100%", height: "450px", borderRadius: "20px", overflow: "hidden", border: "1px solid var(--border-subtle)", boxShadow: "var(--shadow-md)" }}>
            <iframe 
              src={storeInfo.maps_embed || `https://www.google.com/maps?q=${encodeURIComponent(storeInfo.address || '')}&output=embed`}
              width="100%" 
              height="100%" 
              style={{ border: 0 }} 
